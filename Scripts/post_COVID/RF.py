...
def fit_and_predict(df, model, data_deal, train_datadict, pred_start, pred_end, pred_horizon, random_state):
    ...
    print("df_test shape = ", df_test.shape,"for df_test, min = ", df_test.index.min(),", max = ",df_test.index.max())
    ...
    print("y_test_hat shape = ", y_test_hat.shape)
...
for i_date in range(len(rolling_dates)-1):
    ...
    for bst in range(bootstrap_times):
        ...
        print("y_true shape = ", y_true.shape,"  y_test shape = ", y_pred.shape)
