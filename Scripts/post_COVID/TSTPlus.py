...
############################################### data preparation ##########################################
model_name = 'TSTPlus_v3_nontuning_rolling'
mode = 'test8_2023'
...
origin_path = pwd
path = origin_path + '/Data/ILI_data_2023.csv'
...
date_add_list = pd.date_range(start='2024-03-17', periods=pred_stamp-2, freq='7D')
for index_date in date_add_list:
    df.loc[index_date,:] = np.array([0,0,0,0,0,0,0,0])

############################################### modeling ##########################################
study = joblib.load("./model_hyperparam/Normal_TSTPlus.pkl") #load
...
def one_rolling(df, test_start, test_end, pred_horizon, exp_mode = True, bootstrap_times = 100, random_state = None):
    df_t = deepcopy(df.loc[df.index<=pd.to_datetime(test_end),:])
    test_size = df_t.loc[df_t.index > test_start,:].shape[0]
    
    df_test = deepcopy(df_t.loc[df_t.index >= test_start,:])
...
df_test_total = deepcopy(df.loc[df.index >= test_start_date,:])
